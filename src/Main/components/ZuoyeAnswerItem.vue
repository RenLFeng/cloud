    ...
    <div>
        <div class="zacontainer">
            <div class="zaavatar">
                ...
                <div class="zabtnmore zatopmargin" v-if="showsubmitnum"><div class="zatopnumbtn">{{resultitem.submitnum}}次提交</div></div>
                <div class="zaavatartext zatopmargin">{{resultitem.username}}</div>
            </div>


            <TextEllipsis :text="resultitem.ztext" :height="50"  :isLimitHeight="isLimitHeight" class="zatextmain">
                <!--<span slot="before" class="h-tag h-tag-red">new</span> -->
                <template slot="more"><span>...</span><span class="zatextviewmore" @click="isLimitHeight=false">查看更多</span></template>
                <span slot="after"  v-if="!isLimitHeight" @click="isLimitHeight=true"><span class="zatextviewmore">收起</span></span>
            </TextEllipsis>

            <FileAttachList :isupload="isupload" :localfiles="resultitem.localfiles" class="falist"></FileAttachList>

            <div class="zasubmittime">
                {{submittimedesc}}
            </div>
    ...
    export default {
        ...
        ,data(){
            return {
                isupload:false,
                localfiles:[
                    {imgsrc:require('../../assets/zuoye_icon.png')}
                ],
                text: '',
                isLimitHeight: true
            }
