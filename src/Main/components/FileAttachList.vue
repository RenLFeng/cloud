    ...
    <div class="flist-container">
        ...
        <ul class="flist-ul">
            <li v-for="(fitem, findex) in localfiles" class="imgblock" :class="{'liupload':isupload,'imgblocksmall':!isupload}">
                ...
                <div v-else-if="isupload" class="uploadbg uploadbgtransparent">

                </div>

                <div v-if="isupload" class="delbtn" @click="delfileindex(findex)">
                    <div class="delbtntext">
                        x
                    </div>
                </div>
            ...
            <li v-if="isupload" class="imgblock liupload">
                <div class="blockborder" @click="onbtnupload">
                    <div class="imgblocktext textadd ">
                        +
                    </div>
                </div>
            </li>
        ...
        <input ref="uploadfilebtn" type="file" name="file"  @change="uploadChange" style="display:none">
    </div>
</template>
        ...
        ,methods:{
            ...
            uploadChange(event){
                if (event.target.files.length > 0) {
                    ...
                    var vo = {};
                    vo.file = file;
                    vo.ftype = commontools.fileGetType(file.type);
                    vo.mimetype = file.type;
                    if (vo.ftype == 'img'){
                        vo.imgsrc = URL.createObjectURL(file);
                    }
            ...
            ,getimgico(fitem){
                var url = require('../../assets/file-default.png');
                //! todo , get img from ftype.
                return url;
            }
            ,delfileindex(findex){
