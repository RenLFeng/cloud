<template>
    <div>
        ...
        -->
            ...
            <div v-infinite-scroll="loadMoreFile"
                 infinite-scroll-disabled="loadingState"
                 infinite-scroll-distance="10"
            >
                <div v-for="(fitem,selindex) in files" v-bind:key="selindex">
                    <BankeFileSimple :fileitem="files[selindex]" @editclick="oneditclick" @normalclick="onviewfile"></BankeFileSimple>
                </div>
            </div>
            <div v-if="filesempty" class="tc emptydesc">
                {{liststatedesc}}
            </div>
        </div>

        <input ref="uploadfilebtn" type="file" name="file" class="upload__input" @change="uploadChange" style="display:none">
    </div>
</template>
