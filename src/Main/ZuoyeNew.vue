<template>
    <div>
        ...
        <div>
            ...
            <mt-cell title="设置最晚提交时间"><mt-switch v-model="hassubmittime"></mt-switch></mt-cell>
            <mt-cell v-if="hassubmittime" title="最晚提交时间" @click.native="onTimePicker">{{submittimedesc}}</mt-cell>
            <mt-cell v-if="hassubmittime" title="允许超时提交作业"><mt-switch v-model="allowpasstime"></mt-switch></mt-cell>
...
<script>
    ...
    export default {
        ...
        ,methods:{
            ...
            ,onZDetailChange(){
                var maxlen = 6;
                var zt = this.zdetail.ztext;
                if (zt && zt.length > maxlen){
                    zt = zt.substr(0, maxlen);
                    zt += '...';
                }
                this.zuoyeitem.detaildesc = zt;
            }
            ,defaultzuoyename(){
               ...
               // return '';
                ...
                return commontools.sprintf(szfmt, nyear, t.getMonth()+1, t.getDate(),
        ...
        ,created(){
           ...
           // console.log(dd);
            if (dd && dd.editingZuoye){
                ...
                Indicator.open('加载中');
                this.$http.post('/api/api/bankezuoyedetail',
                    {zuoyeid:this.zuoyeitem.id}
                ).then((res)=>{
                    Indicator.close();
                    if (res.data.code == 0){
