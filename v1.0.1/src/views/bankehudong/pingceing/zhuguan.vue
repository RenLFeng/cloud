<template>
  <div class="footer-item-z">
    ...
    >
      ...
      -->
      <div class="mydraw-main"  id="mydraw" ref="drawwrap"
        @touchstart="cwraptstart($event)" @touchend="cwraptend($event)" @touchmove="cwraptmove($event)"
      >
        <canvas ref="drawcanvas" :style="drawcstyle"
                id="mycanvas"
                @touchstart="canvaststart($event)" @touchend="canvastend($event)" @touchmove="canvastmove($event)"
        ></canvas>
      </div>
      <div class="tool-main">
        <ul class="clearfix">
    ...
    uploadChange(e) {
     ...
     // this.uploadImg(file);
      ...
      uploadimgbase64(){
        ...
        Indicator.open('上传中...');
        this.$http.post('/api/pingce/savejpeg', {
   ...
   //   const preData = this.context.getImageData(0, 0, 600, 400);
    ...
    canvasUp(e) {
      ...
      touchgetmove(curtouches, lasttouches, checkshrehold){
          ...
          let shrehold = 1;
          if (!checkshrehold || (Math.abs(dx) >= shrehold || Math.abs(dy) >= shrehold))
          {
              return {
                  action:'move',
                  dx:dx,
      ...
      detectGesture(curtouches, lasttouches, usegesture){
        if (curtouches.length > 1 && lasttouches.length > 1){
            ...
            let xdist = Math.abs(curtouches[0].clientX - curtouches[1].clientX) - Math.abs(lasttouches[0].clientX-lasttouches[1].clientX);
            let ydist = Math.abs(curtouches[0].clientY - curtouches[1].clientY) - Math.abs(lasttouches[0].clientY - lasttouches[1].clientY);
           ...
           // shrehold = 10 * 10;
            ...
            if (usegesture && curgesture && curgesture.action != usegesture.action){
               // return null;
      ...
      cdrawsetpos(cleft, ctop){
          let widthrange = this.drawdata.cwidth * this.drawdata.scale - this.drawdata.wrapwidth;
          let heightrange = this.drawdata.cheight * this.drawdata.scale -  this.drawdata.wrapheight;
          if (widthrange < 0){
              widthrange = 0;
          }
          if (heightrange < 0){
      ...
      cwraptmove(e){
        if (this.drawdata.touchmode == 'multi'){
            let gesture = this.detectGesture(e.touches, this.drawdata.lasttouches, this.drawdata.usegesture);
     ...
     //   console.log(e);
      ...
      wraptocanvas(pos){
        //! 坐标转换， wrap坐标转换为canvas坐标
          if (pos.x){
              let rret = {x:0, y:0};
              rret.x = (pos.x-this.drawdata.cleft) - (this.drawdata.cwidth / 2) * (1.0 - this.drawdata.scale);
              rret.y = (pos.y - this.drawdata.ctop) - (this.drawdata.cheight / 2) * (1.0 - this.drawdata.scale);
    ...
    controlCanvas(action) {
      ...
      cdrawrepos(){
       // if (this.drawdata.wrapwidth > 0)
        {
            this.drawdata.cleft = (this.drawdata.wrapwidth - this.drawdata.cwidth) / 2;
            this.drawdata.ctop = (this.drawdata.wrapheight - this.drawdata.cheight) / 2;
      ...
      cdrawsetimg(url){
        ...
        img.onload = ()=>{
           ...
           // maxwidth = maxheight = 100;
            ...
            this.cdrawsetfit();
            //this.cdrawrepos();
            this.drawdata.ctx.drawImage(img, 0, 0, img.width, img.height,  0, 0, width, height);
        }
      },
    // 生成图片
    getImage() {
    ...
    sumint() {
      ...
      if (this.imgUrl.length <= 1 ){
          if (this.pingceData.editimgurl && this.pingceData.editimgurl.indexOf('base64,') > 0){
