        ...
        <div v-if="classData.length">
          <div class="item" v-for="(v,i) in classData" :key="i">
            ...
            <div class="item-info">
              ...
              <p  v-show="isJoin" class="fontsmall nub">班课人数:&nbsp;{{v.membernum}}</p>
              <p v-show="!isJoin" class="fontsmall nub">在班课中得分:&nbsp;{{v.allScore}}</p>
            </div>
          </div>
        </div>
            ...
            if (res.data.data.length) {
              if (!type) {
                  for(let item of res.data.data){
                    item.allScore=item.score1+item.score2+item.score3+item.score4+item.score5;
                }
